...
public class OldRecommentAlgorithm {
    ...
    public void buildRecommendList(Map<Integer,Map<Integer,Double>> userSimMap,Integer movieNumber) throws IOException {
        ...
        FileWriter fileWriter = new FileWriter("D:\\OldRecommentAlgorithm\\candidacyTop"+movieNumber+".txt", true);
        ...
        while (iteratorMap.hasNext()){
            ...
            System.out.println("用户"+tempUserId);
            if(tempUserId != 4447){
                continue;
            }
            //该用户看过的电影及评分
            ...
            for(Integer simUserId : tempSimUserList){
                ...
                System.out.println("相似用户"+simUserId+"-----该相似用户与用户之间的相似度"+tempSim+"----该相似用户的平均评分"+averageRating(simUserId));
                tempSumSim += tempSim;
                //该相似用户看过的电影以及评分
                ...
                while (tempIteratorSimUserRatingMap.hasNext()){
                    ...
                    System.out.println("tempMovieNewRating:"+tempMovieNewRating);
                    ...
                    System.out.println("推荐电影"+tempEntrySimUserRatingMap.getKey()+"------相似用户对该电影的评分"+simUserRatingMap.get(tempEntrySimUserRatingMap.getKey())+"----第一次计算后的评分"+tempRecommentMovieMap.get(tempEntrySimUserRatingMap.getKey()));
                }
                simUserRatingMap = null;
            }
            System.out.println("相似度综合"+tempSumSim);
            //重新计算该用户推荐列表中的评分
            ...
            while (tempIteratorRecommentMovieMap.hasNext()){
                ...
                System.out.println("----推荐电影"+tempEntryRecommentMovieMap.getKey()+"-------第二次计算该电影的评分"+averageRating(tempUserId)+(tempEntryRecommentMovieMap.getValue()/tempSumSim));
            }
